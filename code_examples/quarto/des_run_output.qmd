...
The median wait for registration across `python g.number_of_runs` runs was `python df_trial_results['Mean Queue Time Reg'].median():.1f` minutes.

The median wait for booking a test was `python df_trial_results['Mean Queue Time Book Test'].median():.1f` minutes.

The median wait for callers to have their call answered was `df_trial_results['Mean Queue Time Call'].median():.1f` minutes.
...
The median wait for a GP across `python g.number_of_runs` was `python df_trial_results['Mean Queue Time GP'].median():.1f` minutes.

The median utilisation for a GP across `python g.number_of_runs` was `df_trial_results['GP Utilisation - Percentage'].median()`%.

## Trial Averages

```{python}
#| label: nhs-colour-sequence-setup
###########################################################
###########################################################
