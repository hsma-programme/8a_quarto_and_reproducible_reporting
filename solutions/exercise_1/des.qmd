---
title: Walk-in GP Clinic - Discrete Event Simulation Results
format:
    html:
        toc: true
        code-fold: true
...
The median wait for registration across `{python} g.number_of_runs` runs was `{python} f"{df_trial_results['Mean Queue Time Reg'].median():.1f}"` minutes.

The median wait for booking a test was `{python} f"{df_trial_results['Mean Queue Time Book Test'].median():.1f}"` minutes.

The median wait for callers to have their call answered was `{python} f"{df_trial_results['Mean Queue Time Call'].median():.1f}"` minutes.

The median utilisation for a receptionist across `{python} g.number_of_runs` runs was `{python} df_trial_results['Receptionist Utilisation - Percentage'].median()`%.

### GP Stats

The median wait for a GP across `{python} g.number_of_runs` runs was `{python} f"{df_trial_results['Mean Queue Time GP'].median():.1f}"` minutes.
...
### Caller-level results

```{python}
#| label: caller-results
show(caller_results)
```
...
### Average waits per stage per run
...
### Resource utilisation by resource
...
calls_answered_df_wide
```

### Percentage of Calls Answered - by run
...
### Call Answering by Arrival Time
